...
class SpyViewer {
public:
	SpyViewer(std::shared_ptr<ProtocolGame> client);
	virtual ~SpyViewer();
	...
	void resetCastOwner();
	void addViewer(std::shared_ptr<ProtocolGame> client);
	void removeViewer(std::shared_ptr<ProtocolGame> client);
	...
	void sendUpdateTileCreature(const Position &pos, int32_t stackpos, std::shared_ptr<Creature> creature);
